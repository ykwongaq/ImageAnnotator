...
from server.util import decode_image_url, JsonUtil, get_resource_path, coco_mask_to_rle, encode_to_coco_mask
from server.segmentation import CoralSegmentation
from server.dataset import Dataset, DataFilter
...
def gen_annotation_file(image, output_file):
    masks_json = coral_segmentation.generate_masks_json(image)
    for mask in masks_json:
        mask["segmentation"] = encode_to_coco_mask(mask["segmentation"])
    image_json = json_util.gen_image_json(image)
    image_json["filename"] = os.path.basename(output_file) 
    output_json = {}
    output_json["image"] = image_json
    output_json["annotations"] = masks_json
    print(f"Saving annotation to {output_file}")
    json_util.save_json(output_json, output_file)
...
def get_data(idx: int):
    ...
    # logger.info(f"Annotation {annotations} ...")
...
def update_config(config):
    ...
    data_filter.set_area_limit(config["area_limit"])
    data_filter.set_iou_limit(config["iou_limit"])
    data_filter.set_predict_iou_limit(config["predicted_iou_limit"])
