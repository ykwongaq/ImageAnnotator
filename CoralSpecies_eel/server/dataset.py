...
from .util.general import load_image_from_content, load_numpy_from_content, load_json_from_content, time_it
...
class DataFilter:
    ...
    def __init__(self, area_limit:int = 0.001, iou_limit: float = 0.5, predict_iou_limit = 0.5):
    ...
    @time_it
    def filtered_by_predicted_iou(self, annotations: List, predicted_iou_limit: float) -> Set:
        filtered_index = set()
    ...
    def filter_annotations(self, annotations: List[Dict]) -> List[Dict]:
        ...
        filtered_indices_by_iou = self.filtered_by_predicted_iou(annotations, self.iou_limit)
        filtered_indices_by_predicted_iou = self.filtered_by_iou(annotations, self.predict_iou_limit)
        filtered_indices = filtered_indices_by_area & filtered_indices_by_iou & filtered_indices_by_predicted_iou

        return [annotations[idx] for idx in filtered_indices]
...
class Dataset:
    ...
    def get_data(self, idx: int) -> Data:
        if idx < 0 or idx >= len(self.data_list):
            self.logger.error(f"Invalid index: {idx} out of range [0, {len(self.data_list)})")
            return None
